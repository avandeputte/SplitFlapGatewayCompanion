...
from app.zonecanvas import ZoneCanvas, _rgb, _FACES, _FACE_W  # noqa: E402  (after the path insert)
...
class Cap(ZoneCanvas):
    """The gallery's render surface: app.zonecanvas.ZoneCanvas (the shared ops->PIL
    shim + borrowed text toolkit) plus the harness's fake microSD card, so sd apps
    render sample photos. Keep drawing behavior in ZoneCanvas — edits there serve
    the Zones feature and this gallery alike."""
