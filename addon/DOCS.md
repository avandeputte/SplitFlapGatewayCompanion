...
| Tool | Does |
|---|---|
| `get_display` | What's on the flaps right now, row by row |
| `show_message` | Put text on the board (centred, word-wrapped); stops any running app |
...
- **The web UI is unauthenticated**, as it always was — ingress means HA is the
...
- The add-on stores its settings in `/data`, and (on Gateway 3.1+) mirrors them to
