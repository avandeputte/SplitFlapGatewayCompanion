...
def test_scoreboard_refetches_when_the_follow_changes():
    ...
    m.fetch_canvas.__dict__.pop("_state", None)
    cv = _Cv()
    m.fetch_canvas({"follow": "nba"}, cv)          # first slate
    m.fetch_canvas({"follow": "nba"}, cv)          # same -> served from cache
    m.fetch_canvas({"follow": "epl:ARS"}, cv)      # changed -> refetch at once
...
def test_a_missing_logo_falls_back_to_a_color_chip():
    ...
    cv = _Cv()
    m.fetch_canvas({"follow": "nba"}, cv)
    assert m.fetch_canvas.__dict__["_state"]["sheet"] == []    # nothing uploaded
