...
def test_canvas_branch_pushes_a_panel_sized_frame_at_every_size():
    for w, h in [(256, 64), (128, 64), (96, 48), (64, 32)]:
        cap = _Cap(w, h)
        hold = HOL.fetch(_SETTINGS, None, lambda: 2, lambda: 20, canvas=cap, i18n=None)
...
def test_canvas_slideshow_advances_through_the_holidays():
    cap = _Cap(128, 64)
    HOL._render_canvas.__dict__.pop("_state", None)         # fresh slideshow
    seen = []
    for _ in range(4):
        HOL.fetch(_SETTINGS, None, lambda: 2, lambda: 20, canvas=cap, i18n=None)
...
def test_flap_branch_still_returns_pages():
    pages = HOL.fetch(_SETTINGS, lambda *a: " | ".join(str(x) for x in a),
                      lambda: 2, lambda: 20, canvas=None, i18n=None)
...
def test_no_holidays_shows_a_message_not_a_crash(monkeypatch):
    ...
    cap = _Cap(128, 64)
    hold = HOL.fetch({}, None, lambda: 2, lambda: 20, canvas=cap, i18n=None)
