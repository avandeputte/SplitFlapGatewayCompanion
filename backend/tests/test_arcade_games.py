...
def test_snake_steers_and_never_reverses():
    ...
    st = app._state._st
    st["dir"] = "r"
    app.fetch_matrix({"speed": "8"}, cv, controls=_Ctl("down", presses=2))
    assert st["dir"] == "d"
    app.fetch_matrix({"speed": "8"}, cv, controls=_Ctl("up", presses=3))
    assert st["dir"] == "u" or st["dir"] == "d"          # up is a reverse only mid-run
    st["dir"] = "r"
    app.fetch_matrix({"speed": "8"}, cv, controls=_Ctl("left", presses=4))
...
def test_snake_dies_into_gameover_and_restarts(gw_calls):
    ...
    app.fetch_matrix({"speed": "8"}, cv, controls=_Ctl(presses=1))
    assert st["phase"] == "gameover"
    f0 = st["fade"]
    for _ in range(3):
        app.fetch_matrix({"speed": "8"}, cv, controls=_Ctl(presses=1))
    assert st["fade"] > f0                               # fading while it waits
    app.fetch_matrix({"speed": "8"}, cv, controls=_Ctl(presses=2))   # any key
...
def test_flappy_press_edge_flaps_once():
    ...
    st = app._state._st
    st["y"], st["vy"] = 30.0, 0.0
    app.fetch_matrix({"speed": "5"}, cv, controls=_Ctl(presses=2))    # fresh press: flap
    assert st["vy"] < 0
    vy_after_flap = st["vy"]
    app.fetch_matrix({"speed": "5"}, cv, controls=_Ctl(presses=2))    # held: no new flap
...
def test_flappy_hits_the_ground_and_restarts():
    ...
    assert st["phase"] == "gameover"
    app.fetch_matrix({"speed": "5"}, cv, controls=_Ctl(presses=1))    # frozen
...
def test_breakout_paddle_steers_and_up_launches():
    ...
    x0 = st["px"]
    app.fetch_matrix({"speed": "5"}, cv, controls=_Ctl("right", presses=2))
    assert st["px"] > x0
...
def test_breakout_brick_hit_scores_and_life_loss_holds_ready():
    ...
    app.fetch_matrix({"speed": "5"}, cv, controls=_Ctl(presses=1))    # arms the freeze, draws READY
    app.fetch_matrix({"speed": "5"}, cv, controls=_Ctl(presses=2))    # a fresh key resumes
