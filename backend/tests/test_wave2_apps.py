...
def test_pong_player_paddle_follows_held_direction():
    ...
    y0 = st["ly"]
    for _ in range(3):
        app.fetch_canvas({"speed": "5"}, cv, controls=_Ctl("down", presses=1))
    assert st["ly"] > y0
    for _ in range(6):
        app.fetch_canvas({"speed": "5"}, cv, controls=_Ctl("up", presses=1))
    assert st["ly"] < y0
...
def test_invaders_tap_fires_one_bolt_at_a_time():
    ...
    app.fetch_canvas({"speed": "5"}, cv, controls=_Ctl(events=["start"], presses=1))
    st = app._state._st
    app.fetch_canvas({"speed": "5"}, cv, controls=_Ctl(taps=["up"], presses=2))
...
def test_invaders_bomb_costs_a_life_and_holds_ready():
    ...
    app.fetch_canvas({"speed": "5"}, cv, controls=_Ctl(presses=1))    # arms the freeze
    app.fetch_canvas({"speed": "5"}, cv, controls=_Ctl(presses=2))    # fresh key resumes
...
def test_sand_player_steers_the_spout_and_bursts():
    ...
    x0 = st["pour_x"]
    for _ in range(4):
        app.fetch_canvas({"speed": "5"}, cv, controls=_Ctl("left", presses=1))
    assert st["pour_x"] < x0
    n0 = len(st["air"]) + st["settled"]
    app.fetch_canvas({"speed": "5"}, cv, controls=_Ctl(presses=2))   # fresh press: burst
...
def test_sensor_graph_samples_and_frames(gw_calls):
    ...
    for v in (21.0, 21.4, 21.9):
        app.fetch_canvas(cfg, cv, get_ha_states=lambda v=v: _states(v))
    st = app.fetch_canvas._state
...
def test_sensor_graph_rotates_a_board():
    ...
    hold = app.fetch_canvas(cfg, cv, get_ha_states=lambda: both)
    st = app.fetch_canvas._state
...
def test_sensor_graph_waits_out_non_numeric_states(gw_calls):
    ...
    app.fetch_canvas(cfg, cv, get_ha_states=lambda: [
...
def test_sensor_graph_seeds_the_window_from_ha_history(gw_calls):
    ...
    app.fetch_canvas({"config": "sensor.office_temp | Office", "window": "60"}, cv,
...
def test_sensor_graph_survives_history_failure_and_reseeds_grown_windows(gw_calls):
    ...
    app.fetch_canvas({"config": "sensor.office_temp"}, cv,
    ...
    app.fetch_canvas({"config": "sensor.office_temp", "window": "120"}, cv,
...
def test_sensor_graph_threshold_polarity_colors_the_card(gw_calls):
    ...
    app.fetch_canvas({"config": "sensor.co2 | CO2 | <500,1000"}, cv,
...
def test_sensor_graph_without_ha_shows_notice(gw_calls):
    app = load_app("canvas-sensor-graph")
    cv = _cv()
    app.fetch_canvas({"config": "sensor.x"}, cv)         # no get_ha_states at all
    assert any("/api/canvas/frame" in c[1] for c in gw_calls)
