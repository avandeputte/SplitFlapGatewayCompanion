...
// ---- overlay ticker (Compose, Matrix panels only) --------------------------
// A line the panel scrolls over whatever app or playlist is running, until cleared. It
// lives in the Compose tab and is shown only on a canvas wall whose firmware advertises
// the overlay (GET /api/panel/caps). The panel's own transitions and its animation and
// font libraries are the gateway's to manage, so the companion no longer mirrors them.
async function syncOverlayCard() {
  const card = $("composeOverlay");
  if (!card) return;
  if (!CANVAS) { card.classList.add("hidden"); return; }
  try {
    const caps = await api("/api/panel/caps");
    card.classList.toggle("hidden", !caps.overlay);
  } catch { card.classList.add("hidden"); }        // not a canvas wall / no overlay
}
