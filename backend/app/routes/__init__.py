...
through it at REQUEST time. That late binding is not a convenience — it is
load-bearing: a test that monkeypatches ``main.vestaboard_key`` must be what
/local-api checks, and ``main._companion_url`` is set by the lifespan AFTER the
routers are built, so capturing it at import would freeze it empty.
...
House rule, pinned by test_display.py: a route resolves
