...
- Playlists "+ Message" now opens the full Compose-style editor (grid geometry, color swatches,
  Center, transition style/speed) in a compact modal, replacing the plain text field. Colors and
  style/speed are saved with the playlist.
...
- Per-screen dwell for apps with multiple predefined screens: Dashboard (Time / Weather),
  Formula 1 (Next race / Standings) and Trivia (Question / Answer) get per-screen "seconds"
  settings, each defaulting to the app's old Delay-Between-Pages value.
...
- Dashboard honors the Temperature Unit and the Time format on split-flap walls (the flap view had
  hard-coded °F/mph and language-based 12h/24h).
- Weather wind now follows the temperature unit — labelled, mph on imperial / km/h on metric.
- New "Check for updates" setting (⚙ Tools): checks GitHub for a newer release and shows a notice
  with a release-notes link on every page.
