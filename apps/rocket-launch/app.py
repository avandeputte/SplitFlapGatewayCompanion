...
def fetch(settings, format_lines, get_rows, get_cols):
    ...
    try:
        ...
        if not res:
            return [format_lines('NEXT LAUNCH', 'NONE', 'SCHEDULED')]
        r = res[0]
        ...
        if net:
            try:
                ...
                if secs <= 0:
                    ...
                    cd = f'IN {d}D {h}H' if d else (f'IN {h}H {m}M' if h else f'IN {m}M')
            except ValueError:
                cd = ''
        if rows == 1:
            return [f'{rocket} {cd}'[:cols].center(cols)]
        if rows == 2:
            return [format_lines('NEXT LAUNCH', rocket), format_lines(mission, cd)]
        return [format_lines('NEXT LAUNCH', rocket, cd), format_lines('MISSION', mission, cd)]
    except Exception:
        return [format_lines('NEXT LAUNCH', 'OFFLINE', '')]
