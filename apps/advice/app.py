...
_TEXT = (238, 238, 244)       # the advice itself
...
def _cv_header(canvas, draw, label):
    ...
    x = 3
    if W >= 96 and H >= 48:
        x += _cv_motif(canvas, draw, 3, 2, hh + 2) + 4
...
def fetch_matrix(settings, canvas):
    ...
    if st['data'] is None or (st['page'] == 0 and now - st['ts'] >= ttl):
        ...
        if got:
            ...
            if st['data'] is None:
                canvas.frame(_cv_message(canvas, ImageDraw, 'Advice', 'Offline'))
                return 60.0
    img, n = _cv_card(canvas, ImageDraw, 'ADVICE', st['data'], st['page'])
    canvas.frame(img)
    st['page'] = (st['page'] + 1) % n
    if n > 1:
        try:
