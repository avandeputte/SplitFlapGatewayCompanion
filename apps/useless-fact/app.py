...
_DIM = (150, 150, 158)        # the bulb's base
...
def _ops_motif(canvas, x, y, s):
    """The lightbulb — glass, a gray base and three rays — as geometry ops (the
    gtext card's twin of _cv_motif)."""
    d = s * 0.62
    bx = x + (s - d) / 2.0
    ray = max(1, int(s * 0.05))
    canvas.circle(bx + d / 2.0, y + s * 0.16 + d / 2.0, d / 2.0, color=_ACCENT, fill=True)
    canvas.rect(x + s * 0.38, y + s * 0.80, s * 0.22, s * 0.18, color=_DIM, fill=True)
    cx = x + s / 2.0
    canvas.line(cx, y, cx, y + max(1.0, s * 0.08), color=_ACCENT, t=ray)
    canvas.line(x + s * 0.06, y + s * 0.18, x + s * 0.16, y + s * 0.28, color=_ACCENT, t=ray)
    canvas.line(x + s * 0.94, y + s * 0.18, x + s * 0.84, y + s * 0.28, color=_ACCENT, t=ray)
...
def _cv_ops(canvas, label, body):
    ...
    hundred bytes a frame. Same bulb+label header over a rule, then the fact as big
...
def fetch_canvas(settings, canvas, i18n=None):
    ...
    if st['data'] is None or (st['page'] == 0 and now - st['ts'] >= ttl):
        ...
        if got:
            ...
            if st['data'] is None:
                canvas.frame(canvas.message(t('Random fact'), t('Offline')))
                return 60.0
    W, H = canvas.width, canvas.height
    if getattr(canvas, "can_gtext", False) and H >= 96:
        _cv_ops(canvas, t('Did you know').upper(), st['data'])
        st['page'] = 0
